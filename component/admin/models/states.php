...
JLoader::import('jab.base.model.list');
 ...
 * @since       1.0.0
 */
class JabModelStates extends JabBaseModelList
{
	/**
	 * Context for session
	 *
	...
	protected function populateState($ordering = 'ordering', $direction = 'ASC')
	{
		...
		$filterState = $this->getUserStateFromRequest($this->context . '.filter_state', 'filter_published');
	...
	protected function getListQuery()
	{
		...
		{
			$query->where('st.state = ' . (int) $published);
		}
		elseif ($published === '')
		{
			$query->where('(st.state IN (0, 1))');
		}
		...
		// Filter: state
		$states = $this->multipleSanitised($this->getState('filter.state'));
		...
		if (!empty($states))
		{
			if (count($states) == 1)
			{
				if (!empty($states[0]))
				{
					$query->where('st.state = ' . (int) $states[0]);
				}
			}
			else
			{
				$query->where('st.state IN (' . implode(',', $states) . ')');
			}
		}
